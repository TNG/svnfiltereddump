...
def _parse_command_line(command_line):
    ...
    parser.add_option(
        ...
        help=(
            "Squash the revision history below the given revision. "
            + "Generate artificial first revision with represents the given input revision."
        ),
        metavar='NUMBER'
    )
    parser.add_option(
        ...
        help=(
            "By default extra nodes are injected to create the parent directories of all paths in the include list. "
            + "Use this option to switch it off."
        ),
    )
    parser.add_option(
        '--drop-old-tags-and-branches', action='store_true', dest='drop_old_tags_and_branches', default=False,
        help=(
            "Use with the --start-rev option. Automatically exclude data in tags and branches directories "
            + "referring to data before and up to the start revision."
        ),
    )
    parser.add_option(
        '--tag-or-branch-dir', action='append', dest='custom_tags_and_branches_dirs', default=[],
        help=(
            "Use with --drop-old-tags-and-branches. Overwrites the list of directory names, which contain tags "
            + "and branches in your repository. Add one --tag-or-branch-dir option for each name you want "
            + "- including 'tags' and 'branches' if you want to extend the original list."
        ),
        metavar='NAME'
    )
    parser.add_option(
        ...
        help=(
            "Use with --drop-old-tags-and-branches. Overwrites the list of directory names, which contain the "
            + "trunk(s) your repository. Add one --tuunk-dir option for each name you want - including 'trunk' "
            + "if you want to extend the original list."
        ),
        metavar='NAME'
    )
    parser.add_option(
...
class Config(object):
    ...
    def __init__(self, command_line):
        ...
        for include_file in options.include_files:
            include_paths += _get_file_as_list(include_file)
        self.include_paths = include_paths

        exclude_paths = options.exclude_paths
        for include_file in options.exclude_files:
            exclude_paths += _get_file_as_list(include_file)
    ...
    def is_path_tag_or_branch(self, path):
        ...
        for element in path.split('/'):
            if element == '':
                continue
            if element in self.trunk_dict:
                return False
            elif element in self.tag_and_branch_dict:
