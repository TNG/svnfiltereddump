...
class SvnRepository(object):
    ...
    def get_dump_file_handle_for_revision(self, rev):
        return CheckedCommandFileHandle(
            ...
            [   # SVN 1.6
                ...
                '^WARNING: will fail\.$',
                # SVN 1.7
                'WARNING 0x0000: Referencing data in revision \d+, which is older than the oldest '
                + 'dumped revision \(r\d+\)',
                'WARNING 0x0000: The range of revisions dumped contained references to copy sources '
                + 'outside that range\.',
                'WARNING 0x0001: Mergeinfo referencing revision\(s\) prior to the oldest dumped revision '
                + '\(r\d+\)\. Loading this dump may result in invalid mergeinfo\.',
                'WARNING 0x0001: The range of revisions dumped contained mergeinfo which reference revisions '
                + 'outside that range\.',
            ]
        )
    ...
    def get_revision_info(self, rev):
        ...
        for property_name in ['svn:author', 'svn:date', 'svn:log']:
            with CheckedCommandFileHandle(
                ['svnlook', 'pg', '--revprop', '-r', str(rev), self.path, property_name]
            ) as fh:
